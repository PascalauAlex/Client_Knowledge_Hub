...
async def upload_document(
    ...
    if doc_type != "report":
        raise HTTPException(
            ...
            "Only 'report' documents are supported for now.",
        )
    ...
    if client.created_by_id != current_user.id:
        raise HTTPException(
            ...
            "You are unauthorized to upload this document to this client",
        )
    ...
    if len(content) > settings.max_file_size:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            "The size of the file cannot be bigger than 100 MB",
        )

    # Process document
    processed_file, filename, extension = await run_in_threadpool(
    ...
    if filename is None or extension is None:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            "The file must have the following extensions: (.pdf, .doc, .docx, .xlsx, .csv)",
        )
    ...
    try:
        ...
        loaded = DocumentLoader(
            extension=extension, file_bytes=processed_file
        ).load_document()
        ...
        await delete_document_s3(filename)
        raise HTTPException(
            status.HTTP_500_INTERNAL_SERVER_ERROR, f"Failed to process document: {err}"
        ) from err
...
async def delete_document(
    ...
    await db.execute(
        sql_delete(models.Document).where(models.Document.id == document_id)
    )
    try:
...
async def get_document_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not fetch document {document.name} from s3",
        )

    if not s3_doc:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail="The current file was not found!"
        )
    structured_invoice = None

    match document.type:
        case "invoice":
            structured_invoice = await run_in_threadpool(
                structured_invoice_summary, document.file, s3_doc
            )
...
@router.get(
    "/{document_id}", status_code=status.HTTP_200_OK, response_model=DocumentResponse
)
