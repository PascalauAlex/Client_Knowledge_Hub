...
async def test_create_client_success(
    ...
    assert db_client.name == "test_client", (
        f"Client was created into database with name: {db_client.name} insted of test_client"
    )
    assert db_client.email == "client@test.com", (
        f"Client was created into database with email: {db_client.email} insted of client@test.com"
    )
    ...
    print(
        f"== DATABASE CLIENT | ENDPOINT RESPONSE ==\n"
...
async def test_fail_create_client(authenticated_client: AsyncClient):
    ...
    response = await authenticated_client.post(
        "/api/clients", json={"email": "client@test.com"}
    )
    ...
    response = await authenticated_client.post(
        "/api/clients", json={"name": "test_client"}
    )
    assert response.status_code == 422
    api_response = response.json()
    detail = api_response.get("detail")[0]
...
async def test_get_user_without_authorization(client: AsyncClient):
    ...
    response = await client.post(
        "/api/clients",
        json={"name": "test_user", "email": "test@client.com"},
    ...
    other_user = await create_test_user(
        client, username="other_user", email="other@example.com"
    )
    ...
    response = await client.get(
        f"/api/clients/{test_client_id}", headers=auth_header(other_user_token)
    )
    print(response.text)
